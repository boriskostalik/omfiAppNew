<script setup>
    ...
    const props=defineProps({publication:{type:Object,required:true},pdfUrl:{type:String,default:''},pdfViewerSrc:{type:String,default:''}})
    defineEmits(['show-bibtex','show-ris'])
    const cleanTitle=(s)=>(s??'').trim().replace(/[„“”"]/g,'').replace(/[‚‘’']/g,'')
    const showEnglishTitle=()=>{const sk=cleanTitle(props.publication?.title);const en=cleanTitle(props.publication?.title_eng);return!!en&&en!==sk}
</script>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div class="space-y-4">
            <div class="flex flex-col gap-3 text-base text-slate-700">
                <div v-if="publication.journal" class="flex gap-2"><strong class="text-slate-900">Časopis:</strong><span>{{ publication.journal }}</span></div>
                <div v-if="publication.year && publication.number" class="flex items-center gap-2">
                    <strong class="text-slate-900">Vydanie:</strong>
                    <Link :href="route('issue.show',{year:publication.year,number:publication.number})"class="inline-flex"@click.stop>
              ...
              <div v-if="publication.firstpage && publication.lastpage" class="flex gap-2"><strong class="text-slate-900">Strany:</strong><span>{{ publication.firstpage }}–{{ publication.lastpage }}</span></div>
              <div v-if="publication.doi" class="flex gap-2">
                <strong class="text-slate-900">DOI:</strong>
                <a :href="'https://doi.org/'+publication.doi" target="_blank" rel="noreferrer" class="text-blue-600 hover:underline" @click.stop>{{ publication.doi }}</a>
              </div>
            </div>
